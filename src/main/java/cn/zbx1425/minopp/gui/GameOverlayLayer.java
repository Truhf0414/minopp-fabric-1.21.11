...
public class GameOverlayLayer {
    ...
    private void renderGameActive(GuiGraphics g, DeltaTracker deltaTracker, BlockEntityMinoTable tableEntity) {
        LocalPlayer player = Minecraft.getInstance().player;
        ...
        if (Minecraft.getInstance().hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (hitState.is(Mino.BLOCK_MINO_TABLE.get())) {
                ...
                if (currentPlayer.equals(cardPlayer)) {
                    ...
                    g.drawString(font, cursorMessage, width / 2 + 12, height / 2 - msgHeight / 2, highlight ? 0xFF222222 : 0xFFFFFFDD);
                    if (isShouting) {
                        g.drawString(font, shoutMessage, width / 2 + 12, height / 2 - msgHeight / 2 + font.lineHeight, highlight ? 0xFF222222 : 0xFFFFFFDD);
                    }
                }
            }
        }
        ...
        if (TurnDeadMan.isAlarmActive()) {
            ...
            g.drawString(font, deadManMessage, -msgWidth / 2, 2, highlight ? 0xFF222222 : 0xFFFFFFDD);
            GuiShim.popMatrix(g);
        }
    }
     ...
     */
    private boolean renderHandCards(GuiGraphics g, DeltaTracker deltaTracker) {
        ...
        for (int i = 0; i < handSize; i++) {
            ...
            if (i == clientHandIndex) {
                ...
                g.drawString(font, cardName, x - font.width(cardName) - 10, y + 10, 0xFFFFFFDD);
            }
...
//            guiGraphics.fill(x + 3, y + 3, x + CARD_WIDTH - 3, y + CARD_HEIGHT - 3, card.suit.color);
            ...
            if (card.family == Card.Family.REVERSE) {
                ...
                g.drawString(font, cardName, 0, 0, 0xFF000000 + colorA * 0x10101);
            }
